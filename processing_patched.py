    ...
    def extract_frames(self, video_id, image_id_offset):
        ...
        with tqdm.tqdm(total=self.num_frames, desc='* Frames', leave=False) as progress:
            ...
            while read:
                ...
                progress.update(1)
                read, frame = self.capture.read()
        assert frame_index > 1, 'Failed to read the frames.'
                    ...
                    if element.tag == 'track':
                        ...
                        for child in element:
                            if child.tag == 'box':
                                ...
                                frame_index = int(attributes['frame']) + self.annotations_offset
                                if frame_index in self.image_ids:
                                    ...
                                    frame_filename = self.frame_filenames[frame_index] if self.relative_frame_id else frame_index
                                    line = frame_filename, track_id, *bounding_box, *rest
